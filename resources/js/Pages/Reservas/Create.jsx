...
export default function Reservar({ auth, clientes, artistas, horasDisponibles }) {
    const [nombre, setNombre] = useState(auth.user ? auth.user.name : '');
    const [apellido, setApellido] = useState(auth.user ? auth.user.email : '');
    const [correo, setCorreo] = useState(auth.user ? auth.user.email : '');
    const [telefono, setTelefono] = useState(auth.user ? auth.user.email : '');
    ...
    const handleDateChange = (e) => {
        ...
        setAvailableHours(horasDisponibles[date] || []);
    };
        ...
        <>
            ...
            <div className='mainReserva'>
                <div className="contenedorReserva">
                    ...
                    <form action="/reservas" method="post" className='formulario'>
                            <h1 className="titulo">Reserva tu cita</h1>
                            <hr className="separadorFormulario"/>
                            <div className='filaUno'>
                                <div className='columnaNombre'>
                                    <label htmlFor="nombre">Nombre: </label>
                                    <input type="text" name="nombre" id="nombre" value={nombre} className='inputs' onChange={e => setNombre(e.target.value)} />
                                </div>
                                <div className='columnaApellido'>
                                    <label htmlFor="apellido">Apellidos: </label>
                                    <input type="text" name="apellido" id="apellido" value={apellido} className='inputs' onChange={e => setApellido(e.target.value)} />
                                </div>
                            </div>
                            <div className='columnas'>
                                <label htmlFor="correo">Correo electrónico: </label>
                                <input type="email" name="correo" id="correo" value={correo} className='inputs' onChange={e => setCorreo(e.target.value)} />
                            </div>
                            <div className='columnas'>
                                <label htmlFor="telefono">Teléfono: </label>
                                <input type="tel" name="telefono" id="telefono" value={telefono} className='inputs' onChange={e => setTelefono(e.target.value)} />
                            </div>
